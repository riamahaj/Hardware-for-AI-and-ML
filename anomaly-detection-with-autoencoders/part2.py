...
scaler.fit(df.drop('anomaly', axis = 1))
...
history = autoencoder.fit(x=train_data, y=train_data, epochs=5, verbose=1,
                          batch_size=256,
                          validation_data=[test_data, test_data], callbacks=[es])
...
#train_events_mse = tf.keras.losses.mean_squared_error(train_data, train_predicted_x).numpy()
...
#normal_events_mse = tf.keras.losses.mean_squared_error(real_x, predicted_x).numpy()
...
#abnormal_events_mse = tf.keras.losses.mean_squared_error(abnormal_x, predicted_x).numpy()
